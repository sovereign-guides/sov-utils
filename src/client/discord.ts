...
export async function handleDiscordRequest(request: Request, env: Env): Promise<Response> {
	if (!request.headers.get("x-signature-ed25519") || !request.headers.get("x-signature-timestamp")) return error(401);
